...
import { AddonModThrdAssignHelper, AddonModThrdAssignSubmissionFormatted } from '../thrdassign-helper';
import { CoreUtils } from '@services/utils/utils';
import { CoreFilepool } from '@services/filepool';
import { CoreGroups } from '@services/groups';
import { AddonModThrdAssignSync, AddonModThrdAssignSyncResult } from '../thrdassign-sync';
import { CoreUser } from '@features/user/services/user';
import { CoreGradesHelper } from '@features/grades/services/grades-helper';
import { CoreCourses } from '@features/courses/services/courses';

/**
 * Handler to prefetch thrdassigns.
 */
@Injectable({ providedIn: 'root' })
export class AddonModThrdAssignPrefetchHandlerService extends CoreCourseActivityPrefetchHandlerBase {

    name = 'AddonModThrdAssign';
    modName = 'thrdassign';
    ...
    async canUseCheckUpdates(module: CoreCourseAnyModuleData, courseId: number): Promise<boolean> {
        ...
        try {
            const thrdassign = await AddonModThrdAssign.getAssignment(courseId, module.id);

            const data = await AddonModThrdAssign.getSubmissions(thrdassign.id, { cmId: module.id });
            if (data.canviewsubmissions) {
                return false;
            }

            // Check if the user can view their own submission.
            await AddonModThrdAssign.getSubmissionStatus(thrdassign.id, { cmId: module.id });
    ...
    async getFiles(module: CoreCourseAnyModuleData, courseId: number): Promise<CoreWSFile[]> {
        ...
        try {
            const thrdassign = await AddonModThrdAssign.getAssignment(courseId, module.id, { siteId });
            // Get intro files, attachments and activity files.
            let files: CoreWSFile[] = thrdassign.introattachments || [];
            files = files.concat(this.getIntroFilesFromInstance(module, thrdassign));
            files = files.concat(thrdassign.activityattachments || []);

            // Now get the files in the submissions.
            const submissionData = await AddonModThrdAssign.getSubmissions(thrdassign.id, { cmId: module.id, siteId });

            if (submissionData.canviewsubmissions) {
                // Teacher, get all submissions.
                const submissions =
                    await AddonModThrdAssignHelper.getSubmissionsUserData(thrdassign, submissionData.submissions, 0, { siteId });
                ...
                const blindMarking = !!thrdassign.blindmarking && !thrdassign.revealidentities;

                const submissionFiles = await this.getSubmissionFiles(thrdassign, userId, blindMarking, false, siteId);
                files = files.concat(submissionFiles);
            }
    ...
    protected async getSubmissionFiles(
        ...
        const submissionStatus = await AddonModThrdAssign.getSubmissionStatusWithRetry(thrdassign, {
            userId: submitId,
            isBlind: blindMarking,
            siteId,
        });
        const userSubmission = AddonModThrdAssign.getSubmissionObjectFromAttempt(thrdassign, submissionStatus.lastattempt);

        // Get intro and activity files from the submission status if it's a student.
        // It's ok if they were already obtained from the thrdassignment instance, they won't be downloaded twice.
        const files: CoreWSFile[] = canViewAllSubmissions ?
            [] :
            (submissionStatus.thrdassignmentdata?.attachments?.intro || [])
                .concat(submissionStatus.thrdassignmentdata?.attachments?.activity || []);
        ...
        if (userSubmission.plugins) {
            ...
            userSubmission.plugins.forEach((plugin) => {
                promises.push(AddonModThrdAssignSubmissionDelegate.getPluginFiles(thrdassign, userSubmission, plugin, siteId));
            });
        }
        ...
        if (submissionStatus.feedback && submissionStatus.feedback.plugins) {
            ...
            submissionStatus.feedback.plugins.forEach((plugin) => {
                promises.push(AddonModThrdAssignFeedbackDelegate.getPluginFiles(thrdassign, userSubmission, plugin, siteId));
            });
        }
    ...
    async invalidateContent(moduleId: number, courseId: number): Promise<void> {
        await AddonModThrdAssign.invalidateContent(moduleId, courseId);
    }
    ...
    protected async prefetchAssign(module: CoreCourseAnyModuleData, courseId: number, siteId: string): Promise<void> {
        ...
        // Get thrdassignment to retrieve all its submissions.
        const thrdassign = await AddonModThrdAssign.getAssignment(courseId, module.id, options);
        const promises: Promise<unknown>[] = [];
        const blindMarking = thrdassign.blindmarking && !thrdassign.revealidentities;

        if (blindMarking) {
            promises.push(
                CoreUtils.ignoreErrors(AddonModThrdAssign.getAssignmentUserMappings(thrdassign.id, -1, modOptions)),
            );
        }

        promises.push(this.prefetchSubmissions(thrdassign, courseId, module.id, userId, siteId));

        promises.push(CoreCourse.getModuleBasicInfoByInstance(thrdassign.id, 'thrdassign', { siteId }));
    ...
    /**
     * Prefetch thrdassign submissions.
     *
     * @param thrdassign Assign.
    ...
    protected async prefetchSubmissions(
        ...
        // Get submissions.
        const submissions = await AddonModThrdAssign.getSubmissions(thrdassign.id, modOptions);
        const promises: Promise<unknown>[] = [];
    ...
    protected async prefetchParticipantSubmissions(
        ...
        const groupInfo = await CoreGroups.getActivityGroupInfo(thrdassign.cmid, false, undefined, siteId);
        ...
        const promises = groupInfo.groups.map((group) =>
            AddonModThrdAssignHelper.getSubmissionsUserData(thrdassign, submissions, group.id, options)
                .then((submissions: AddonModThrdAssignSubmissionFormatted[]) => {
                    ...
                    const subPromises: Promise<unknown>[] = submissions.map((submission) => {
                        ...
                        return this.prefetchSubmission(thrdassign, courseId, moduleId, submissionOptions, true);
                    });

                    subPromises.push(AddonModThrdAssign.getAssignmentGrades(thrdassign.id, modOptions));

                    // Prefetch the submission of the current user even if it does not exist, this will be create it.
                    if (!submissions || !submissions.find((subm: AddonModThrdAssignSubmissionFormatted) => subm.submitid == userId)) {
                        const submissionOptions = {
                            userId,
                            groupId: group.id,
                            readingStrategy: CoreSitesReadingStrategy.ONLY_NETWORK,
                        ...
                        subPromises.push(this.prefetchSubmission(thrdassign, courseId, moduleId, submissionOptions));
                    }

                    return Promise.all(subPromises);
                }).then(async () => {
                    // Participiants already fetched, we don't need to ignore cache now.
                    const participants = await AddonModThrdAssignHelper.getParticipants(thrdassign, group.id, { siteId });
    ...
    protected async prefetchSubmission(
        ...
        const submission = await AddonModThrdAssign.getSubmissionStatusWithRetry(thrdassign, options);
        ...
        try {
            ...
            const blindMarking = !!thrdassign.blindmarking && !thrdassign.revealidentities;
            let userIds: number[] = [];
            const userSubmission = AddonModThrdAssign.getSubmissionObjectFromAttempt(thrdassign, submission.lastattempt);
            ...
            if (submission.lastattempt) {
                ...
                if (userSubmission && userSubmission.id) {
                    ...
                    if (userSubmission.plugins) {
                        userSubmission.plugins.forEach((plugin) => {
                            ...
                            promises.push(
                                AddonModThrdAssignSubmissionDelegate.prefetch(thrdassign, userSubmission, plugin, siteId),
                            );

                            // Prefetch the plugin files.
                            promises.push(
                                AddonModThrdAssignSubmissionDelegate.getPluginFiles(thrdassign, userSubmission, plugin, siteId)
                                    .then((files) =>
                ...
                if (thrdassign.teamsubmission && submission.lastattempt.submissiongroup) {
                    // Prefetch group info.
                    promises.push(CoreGroups.getActivityAllowedGroups(thrdassign.cmid));
                }
            }
            ...
            if (submission.feedback) {
                ...
                if (submission.feedback.plugins && userSubmission && userSubmission.id) {
                    submission.feedback.plugins.forEach((plugin) => {
                        // Prefetch the plugin WS data.
                        promises.push(AddonModThrdAssignFeedbackDelegate.prefetch(thrdassign, userSubmission, plugin, siteId));

                        // Prefetch the plugin files.
                        promises.push(
                            AddonModThrdAssignFeedbackDelegate.getPluginFiles(thrdassign, userSubmission, plugin, siteId)
     ...
     * @returns Promise resolved when done.
     */
    sync(module: CoreCourseAnyModuleData, courseId: number, siteId?: string): Promise<AddonModThrdAssignSyncResult> {
        return AddonModThrdAssignSync.syncAssign(module.instance, siteId);
    }

}
export const AddonModThrdAssignPrefetchHandler = makeSingleton(AddonModThrdAssignPrefetchHandlerService);
