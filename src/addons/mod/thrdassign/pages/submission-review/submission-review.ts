...
import { AddonModThrdAssignListFilterName, AddonModThrdAssignSubmissionsSource } from '../../classes/submissions-source';
import { AddonModThrdAssignSubmissionComponent } from '../../components/submission/submission';
import { AddonModThrdAssign, AddonModThrdAssignAssign } from '../../services/thrdassign';
 ...
 */
@Component({
    selector: 'page-addon-mod-thrdassign-submission-review',
    templateUrl: 'submission-review.html',
})
export class AddonModThrdAssignSubmissionReviewPage implements OnInit, OnDestroy, CanLeave {

    @ViewChild(AddonModThrdAssignSubmissionComponent) submissionComponent?: AddonModThrdAssignSubmissionComponent;

    title = ''; // Title to display.
    submissions?: AddonModThrdAssignSubmissionSwipeItemsManager;
    ...
    protected thrdassign?: AddonModThrdAssignAssign; // The thrdassignment the submission belongs to.
    ...
    constructor(protected route: ActivatedRoute) {
        this.logView = CoreTime.once(() => {
            ...
            CoreAnalytics.logEvent({
                ...
                ws: 'mod_thrdassign_get_submission_status',
                ...
                data: { id, thrdassignid: this.thrdassign.id, category: 'thrdassign' },
                url: `/mod/thrdassign/view.php?id=${this.thrdassign.cmid}&action=grader&${paramName}=${id}`,
            });
        });
    }
    ...
    ngOnInit(): void {
        this.route.queryParams.subscribe((params) => {
            try {
                ...
                const selectedStatus = CoreNavigator.getRouteParam<AddonModThrdAssignListFilterName>('selectedStatus');
                ...
                this.submissions = new AddonModThrdAssignSubmissionSwipeItemsManager(submissionsSource);
    ...
    protected async fetchSubmission(): Promise<void> {
        ...
        this.title = this.thrdassign.name;

        this.blindMarking = !!this.thrdassign.blindmarking && !this.thrdassign.revealidentities;
    ...
    protected async refreshAllData(): Promise<void> {
        ...
        if (this.thrdassign) {
            promises.push(AddonModThrdAssign.invalidateSubmissionData(this.thrdassign.id));
 ...
 */
class AddonModThrdAssignSubmissionSwipeItemsManager extends CoreSwipeNavigationItemsManager {
