...
export class AddonModTHPageProvider {

    static readonly COMPONENT = 'mmaModTHPage';
     ...
     */
    getTHPageData(courseId: number, cmId: number, options: CoreSitesCommonWSOptions = {}): Promise<AddonModPagePage> {
        return this.getTHPageByKey(courseId, 'coursemodule', cmId, options);
    }
     ...
     * @returns Promise resolved when the page is retrieved.
     */
    protected async getTHPageByKey(
        courseId: number,
        key: string,
        value: number,
        ...
        const response = await site.read<AddonModPageGetPagesByCoursesWSResponse>('mod_thpage_get_thpages_by_courses', params, preSets);
        console.log(response);
        const pdfFile = response.thpages.find((page) => page[key] == value);
        console.log(pdfFile);
        if (pdfFile) {
            return pdfFile;
        }
    ...
    invalidateContent(moduleId: number, courseId: number, siteId?: string): Promise<void> {
        ...
        promises.push(CoreFilepool.invalidateFilesByComponent(siteId, AddonModTHPageProvider.COMPONENT, moduleId));
...
export const AddonModTHPage = makeSingleton(AddonModTHPageProvider);
