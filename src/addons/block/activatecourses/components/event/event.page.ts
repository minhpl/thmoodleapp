...
import { AddonCalendar, AddonCalendarEventToDisplay, AddonCalendarProvider } from '@addons/calendar/services/calendar';
import { AddonCalendarSync, AddonCalendarSyncEvents, AddonCalendarSyncProvider } from '@addons/calendar/services/calendar-sync';
import { AddonCalendarEventReminder, AddonCalendarHelper } from '@addons/calendar/services/calendar-helper';
import { AddonCalendarOffline } from '@addons/calendar/services/calendar-offline';
...
export class AddonCalendarEventPage implements OnInit, OnDestroy {
    ...
    mycourse: any = null;
    constructor(
        protected route: ActivatedRoute, private nav: NavController
    ) {
        console.log("xin chao the gioi tuoi dep");

        this.route.queryParams.subscribe(params => {
            this.mycourse = params;
            console.log("xin chao the gioi tuoi dep 2", this.mycourse);
        });
    ...
    async kichhoat() {
        ...
        return site.read<any>('block_th_activatecourses_activate_course', data, preSets).then((result) => {
    ...
    vaokhoahoc() {
        this.nav.pop().then(()=>
            {
                CoreCourseHelper.openCourse(this.mycourse);
            });
    }
